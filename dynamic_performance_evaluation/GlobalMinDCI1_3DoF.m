function DCI_star = GlobalMinDCI1_3DoF(x,xi_ai,xi_pi,gst0,gsli0,M0b_CoM)
...
p_count = 0;
% for ta1=-1.5708:step_a1:1.5708 % not needed!
...
%             [DCI(p_count),E(:,:,p_count)] = CalculateDynamicConditioningIndex_3DoF(Mb,3);
            [DCI(p_count),E(:,:,p_count),P(:,:,p_count),D(:,:,p_count)] = CalculateDynamicConditioningIndex2_3DoF(Mb,3,wx);
        end
    end
% end
DCI_star = min(DCI); 
